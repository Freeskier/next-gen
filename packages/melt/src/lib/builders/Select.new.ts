...
export type SelectProps<T, Multiple extends boolean = false> = Omit<
	PopoverProps,
	"sameWidth"
> & {
	/**
					...
					(acc, curr) => {
						...
						return [
							...acc,
							{
								...
								current: curr.dataset.value === String(this.highlighted),
							},
						];
					},
					[] as Array<{ value: string; typeahead?: string; current: boolean }>,
				);
			},
		}),
	);
			...
			onkeydown: (e: KeyboardEvent) => {
				...
				switch (e.key) {
					...
					default: {
						...
						if (next && next.value) {
							const matchedValue = this.#optionMap.get(next.value);
							if (matchedValue) {
